...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (form.role === 'student'   && !form.university)       e.university = 'University is required';
    if (form.role === 'organizer' && !form.organizationName) e.organizationName = 'Organization is required';
    setErrors(e);
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      const payload = {
        ...
        university: form.university, degreeLevel: form.degreeLevel,
        ...(form.role === 'organizer' && {
          organizerProfile: { organizationName: form.organizationName, emailDomain: form.emailDomain }
        }),
      };
      await register(payload);
      toast.success('Account created! Welcome to Tictify 🎉');
    ...
    <AuthLayout heading="Create account" subheading="Join thousands of students finding opportunities daily">
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 anim-fade-up delay-2">
          ...
          <div>
            <label className="form-label">{form.role === 'student' ? 'University' : 'Organization'}</label>
            {form.role === 'student' ? (
        ...
        <div className="grid grid-cols-2 gap-3 anim-fade-up delay-3">
          <div>
            ...
            <div className="relative">
              <input name="password" type={showPass?'text':'password'}
                value={form.password} onChange={onChange} placeholder="Min 8 chars"
          ...
          <div>
            ...
            <input name="confirmPassword" type="password"
              value={form.confirmPassword} onChange={onChange} placeholder="Repeat"
        ...
        <div className="anim-fade-up delay-4">
          {form.role === 'student' ? (
            <>
              ...
              <select name="degreeLevel" value={form.degreeLevel} onChange={onChange} className="input-field">
                {DEGREE_OPTIONS.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
              </select>
            </>
          ) : (
            <>
              <label className="form-label">Email Domain <span style={{ color:'var(--text-muted)', textTransform:'none', letterSpacing:0, fontWeight:400 }}>(optional)</span></label>
        ...
        <p className="text-center text-sm anim-fade-up delay-5" style={{ color:'var(--text-secondary)' }}>
